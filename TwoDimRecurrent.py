...
class TwoDimCWRNN(TwoDimRecurrent):
    ...
    def get_dropout(self,batchsize = None):
        ...
        if self.droprate is not None:
          ...
          B_U = self.srng.binomial((2, batchsize, self.single_dim), p = retain_prob_U, dtype = theano.config.floatX)
          return [B_W, B_U] 
        else:
          return []
    ...
    # def step(self, x,row_state, col_state,**kwargsdic):
        ...
        # x = x*dropout_matrix[0][0]/self.droprate[0]
        # prev_output_row = prev_output_row * dropout_matrix[1][0] /self.droprate[1]
        # prev_output_col = prev_output_col * dropout_matrix[1][1] /self.droprate[1]
                
        # x_t = K.dot(x, self.W) + self.b  # output_dim, first half for row, the other for col
        # hidden_sum = K.dot(prev_output_row, self.U_row*self._mask) \
                   # + K.dot(prev_output_col, self.U_col*self._mask)
